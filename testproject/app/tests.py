...
class MaintenanceModeMiddlewareTestCase(TestCase):
    ...
    def test_enabled_middleware_with_template(self):
        ...
        # reset INTERNAL_IPS so that the request will appear to come
        # from the outside
        with self.settings(INTERNAL_IPS=()):
            response = self.client.get(self.home_url)
            self.assertContains(
                response, text=self.maintenance_page_text, count=1,
                status_code=503
            )

    def test_middleware_with_non_staff_user(self):
        """
        A logged in user that is not a staff user should see the
        503 message
        """
        self.turn_maintenance_mode_on()
    ...
    def test_middleware_with_staff_user(self):
        """
        A logged in user that _is_ a staff user should be able to use
        the site normally
        """
    ...
    def test_ignored_path(self):
        """
        A path is ignored when applying the maintanance mode and should
        be reachable normally
        """
        self.turn_maintenance_mode_on()
    ...
    def test_django_admin_accessable(self):
        """
        Make sure we can still log into Django admin to turn
        ...
        self.assertEqual(
            ...
            httplib.OK,
            'Unable to reach Django Admin login'
        )
